# 👋 Hi, I’m Damian

🎮 Game developer specialized in **gameplay programming** and passionate about **interactive simulation**.  
I hold a Master’s degree in Game Development from **HEAJ (Namur, Belgium)**, where I worked on complete projects ranging from gameplay programming to applied research in flight simulation.

---

## 🚀 Featured Projects
- **[Mini Airline](https://github.com/TheMakness/MiniAirline)** – A work-in-progress C++/SFML project inspired by *Mini Airways*.
  Its main goal is to strengthen my C++ skills while exploring resource management and optimization mechanics.

  You can follow my progress here -> https://trello.com/b/0BD57TCc/miniairline
  
- **[Research Thesis](https://github.com/TheMakness/FlightSimulation)** – Study of flight simulation models with a focus on player experience.


---

## 🔧 Technical Skills
- **Languages**: C++, C#, Blueprint, Python  
- **Engines & Tools**: Unreal Engine 5, Unity, Git, Perforce  
- **Other**: Physics simulation, Niagara particle systems, internal tool design

---

## 🎯 Interests
- Gameplay systems & systemic design
- Software design
- Simulation and applied research in games
- VFX
- Technical Art
- Graphics Programming  

---

## 📫 Get in Touch
- ✉️ damian.lallement@outlook.fr  
- 💼 https://www.linkedin.com/in/damian-lallement 
- 🎮 [damianlallement.me](https://damianlallement.me)
